                    ...
                    <ul class="collapsible expandable collapsible-dark">
                              ...
                              <table class="table table-striped table-dashboard-two mg-b-0">
                                ...
                                <tbody>
                                  <?php if($jumlah_termin->jumlah < $result->termin) { ?>
                                    <a href="#modaltermin" class="btn btn-success btn-with-icon btn-block rounded-5 modal-trigger"><i class="far fa-file-alt"></i> Tambah Termin</a>
                                  <?php } ?>
                                      ...
                                      <tr>
                                        ...
                                        <td>
                                          <?php if ($cek_termin->jumlah != $cek_jumlah_termin->jumlah) { ?>
                                            <a href="#modalterminedit<?php echo $a->termin; ?>" class="btn btn-success btn-with-icon btn-block rounded-5 modal-trigger"><i class="far fa-file-alt"></i> Edit Termin</a>
                                          <?php } elseif($cek_termin->jumlah == $cek_jumlah_termin->jumlah && $cek_termin->jumlah > 0) { ?>
                                            <?php if($a->status_termin == 0) { ?>
                                              <a href="<?php echo base_url(); ?>program/pengajuan_pembayaran/<?php echo $a->id_kontrak; ?>/<?php echo $a->id_termin; ?>" class="btn btn-success green btn-with-icon btn-block rounded-5"><i class="fas fa-paper-plane"></i> Pengajuan Termin Pembayaran</a>
                                            <?php } elseif($a->status_termin == 1) { ?>
                                              <a href="#" class="btn btn-success green btn-with-icon btn-block rounded-5"><i class="fas fa-exclamation-circle"></i> Pengajuan Termin Pembayaran telah diajukan</a>
                                              <a href="#modalterminedit<?php echo $a->termin; ?>" class="btn btn-success btn-with-icon btn-block rounded-5 modal-trigger"><i class="far fa-file-alt"></i> Edit Termin</a>
                                            <?php } ?>
                                          <?php } else { ?>
                                            <a href="#modalterminedit<?php echo $a->termin; ?>" class="btn btn-success btn-with-icon rounded-5 modal-trigger"><i class="far fa-file-alt"></i> Edit Termin</a>
                                            <a href="<?php echo base_url(); ?>program/hapus_termin/<?php echo $a->id_kontrak; ?>/<?php echo $a->id_termin; ?>" class="btn red btn-with-icon rounded-5" onclick="javascript: return confirm('Yakin akan menghapus data ?')"><i class="fas fa-trash"></i> Hapus Termin</a>
                                      ...
                                      <?php if ($result->termin > 0) { ?>
                                        ...
                                        <?php for ($i=$jumlah_termin->jumlah + 1; $i < $result->termin + 1; $i++) { ?>
                                          <div class="row">
                                              ...
                                              <div class="input-field col s12 l6">
                                                  <input id="a6" type="number" name="jumlah[]" min="1" maxlength="20">
                        ...
                        <li>
                            ...
                            <div class="collapsible-body">
                              <div class="row">
                                    ...
                                    <a href="#modalanggota" class="btn btn-success btn-with-icon  rounded-5 modal-trigger"><i class="fas fa-plus"></i> Tambah Anggota</a>
                                </div>
                              ...
                              <tbody>
                                <?php foreach ($anggota as $a) { ?>
                                  <tr>
                                    ...
                                    <td><a href="<?php echo base_url();?>program/hapus_anggota/<?php echo $result->id_kontrak;?>/<?php echo $a->id_personil; ?>" class="btn red btn-with-icon rounded-5 modal-trigger" onclick="javascript: return confirm('Yakin akan menghapus data ?')"><i class="fas fa-trash"></i> Hapus Anggota</a></td>
                                  </tr>
                                <?php } ?>
                              </tbody>
                              </table>
                              <br>
                              <center>
                                <a href="#modalsk" class="btn blue btn-with-icon rounded-5 modal-trigger"><i class="far fa-file"></i> Upload SK TIM</a>
                              <?php if ($result->file_sk != "") { ?>
                                <a href="<?php echo base_url();?>uploads/sk_tim/<?php echo $result->file_sk;?>" target="_blank" class="btn red btn-with-icon rounded-5 modal-trigger"><i class="far fa-file-alt"></i> View File</a>
                              <?php } ?>
                              </center>
                        ...
                        <li>
                            ...
                            <div class="collapsible-body">
                                    ...
                                    <?php foreach ($result_timeline as $a) { ?>
                                      <tr>
                                        ...
                                        <?php if ($a->status == 0) { ?>
                                          <td>
                                            <center>
                                            <a href="#modaltimelineedit<?php echo $a->id_kegiatan;?>" class="btn blue btn-with-icon modal-trigger"><i class="far fa-file-alt"></i> Edit</a>
                                            <?php $cek = $this->db->query("SELECT * FROM rencana_operasional WHERE id_kegiatan=$a->id_kegiatan")->num_rows();?>
                                            <?php if($cek == 0) { ?>
                                              <a href="<?php echo base_url();?>program/hapus_timeline/<?php echo $result->id_kontrak;?>/<?php echo $a->id_kegiatan; ?>" class="btn red btn-with-icon" onclick="javascript: return confirm('Yakin akan menghapus data ?')"><i class="fas fa-trash"></i> Hapus</a>
                                            <?php } ?>
                                            </center>
                                          </td>
                                        <?php } elseif ($a->status == 1) {?>
                                          <td>
                                            <center>
                                              <h5><span class="label label-table label-warning">Menunggu Approval</strong></span></h5>
                                            </center>
                                          </td>
                                        <?php } elseif ($a->status == 2) {?>
                                          <td>
                                            <center>
                                              <h5><span class="label label-table label-warning">Menunggu Tindak Lanjut Kendala</strong></span></h5>
                                            </center>
                                          </td>
                                        <?php } elseif ($a->status == 4) {?>
                                          <td>
                                            <center>
                                              <h5><span class="label label-table label-success">Approved</strong></span></h5>
                                            </center>
                                          </td>
                                        <?php } ?>
                                      </tr>
                              ...
                              <a href="#modaltimeline" class="btn btn-success btn-with-icon btn-block rounded-5 modal-trigger"><i class="far fa-file-alt"></i> Tambah Timeline</a>
                              <?php foreach ($result_timeline as $a) { ?>
                                        ...
                                        <?php echo form_open_multipart('program/edit_timeline');?>
                                          <div class="row">
                                              <div class="input-field col s12">
                                                  ...
                                                  <select name="termin">
                                                      <option value="<?php echo $a->termin_id; ?>" selected><?php echo "Termin - ".$a->termin_kegiatan." Tanggal : ".$a->tgl_termin; ?></option>
                                                      <?php foreach ($result_termin as $b) { ?>
                                                        <option value="<?php echo $b->id_termin; ?>"><?php echo "Termin - ".$b->termin_kegiatan." Tanggal : ".$a->tgl_termin; ?></option>
                                                      <?php } ?>
                                                  </select>
                                              </div>
                                          </div>
                                          ...
                                          <div class="row">
                                              <div class="input-field col s12">
                                                  ...
                                                  <select name="termin">
                                                      ...
                                                      <?php foreach ($result_termin as $a) { ?>
                                                        <option value="<?php echo $a->id_termin; ?>"><?php echo "Termin - ".$a->termin_kegiatan." Tanggal : ".$a->tgl_termin; ?></option>
                                                      <?php } ?>
                                                  </select>
                                              </div>
                                          </div>
                        ...
                        <li>
                            ...
                            <div class="collapsible-body">
                              <a href="#modalmax" class="btn blue btn-with-icon rounded-5 modal-trigger"><i class="fas fa-clipboard-list"></i> Setting Max. Biaya Operasional</a>
                                ...
                                <tbody>
                                  ...
                                  <?php $total=0;$total_p=0;$total_r=0;$total_s=0; foreach ($ro as $a) { ?>
                                    ...
                                    <tr>
                                      ...
                                      <td><a href="<?php echo base_url();?>program/detail_ro/<?php echo $a->id_kontrak."/".$a->akun;?>" class="btn red btn-with-icon rounded-5"><i class="fas fa-eye"></i> Detail</a></td>
                                    </tr>
