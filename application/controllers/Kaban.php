...
class Kaban extends CI_Controller {
	...
	function index()
	{
		...
		// $data['terkontrak'] = $this->db->query("SELECT SUM(nilai_kontrak) AS jumlah FROM kontrak WHERE YEAR(tgl_akhir) = $tahun")->row();
		// $data['pengeluaran'] = $this->db->query("SELECT SUM(jumlah_realisasi) AS jumlah FROM pengajuan AS p INNER JOIN rencana_operasional AS ro ON p.id_ro = ro.id_ro INNER JOIN kontrak AS k ON ro.id_kontrak = k.id_kontrak WHERE p.status_realisasi = 1 AND YEAR(p.tgl_pengajuan) = $tahun")->row();
		// $data['pelaksana_layanan'] = $this->db->query("SELECT id_rumah_layanan,nama FROM rumah_layanan")->result();
		// $data['satker'] = $this->db->query("SELECT * FROM satker")->result();
		// $data['target'] = $this->m_kontrak->sum_target_all()->row();

		$url_rekap="http://jlt.lemigas.esdm.go.id/db2json.php?qu=select%20*%20from%20ws_v_rekap_lemigas%20WHERE%20tahun=$pilih_tahun";
		$get_url_rekap = file_get_contents($url_rekap);
		$data_json_rekap = json_decode($get_url_rekap);
		//print_r($data_json_rekap);

		$url_target="http://jlt.lemigas.esdm.go.id/db2json.php?qu=SELECT%20SUM(target)%20AS%20jumlah%20FROM%20ws_target%20WHERE%20tahun=$pilih_tahun";
		$get_url_target = file_get_contents($url_target);
		$data_json_target = json_decode($get_url_target);

		//$url_ro="http://jlt.lemigas.esdm.go.id/db2json.php?qu=SELECT%20SUM(mak_nilai)%20AS%20jumlah%20FROM%20ws_rencana_operasional%20WHERE%20YEAR(tgl_ro)=$pilih_tahun%20AND%20status=%27SETUJU%27";
		//$url_ro="http://jlt.lemigas.esdm.go.id/db2json.php?qu=SELECT%20SUM(nilai)%20AS%20jumlah%20FROM%20ws_realisasi_operasional%20WHERE%20YEAR(tanggal)=$pilih_tahun";
		//$url_ro="http://jlt.lemigas.esdm.go.id/db2json.php?qu=SELECT%20SUM(nilai)%20AS%20jumlah%20FROM%20ws_realisasi_operasional%20WHERE%20YEAR(tanggal)=$pilih_tahun%20AND%20jenis_bayar=%27MASTER%27";
		$url_ro="http://jlt.lemigas.esdm.go.id/db2json.php?qu=SELECT%20sum(nilai)%20AS%20jumlah%20FROM%20ws_realisasi_operasional%20WHERE%20YEAR(tanggal)=$pilih_tahun%20AND%20jenis_bayar=%27MASTER%27%20AND%20proses_id%3E0";
		$get_url_ro = file_get_contents($url_ro);
		$data_json_ro = json_decode($get_url_ro);

		$url_kontrak="http://jlt.lemigas.esdm.go.id/db2json.php?qu=SELECT%20SUM(kontrak_nilai_rp)%20AS%20jumlah%20FROM%20ws_kontrak%20WHERE%20YEAR(kontrak_tanggal)=$pilih_tahun";
		$get_url_kontrak = file_get_contents($url_kontrak);
		$data_json_kontrak = json_decode($get_url_kontrak);
		//$url_termin="http://jlt.lemigas.esdm.go.id/db2json.php?qu=SELECT SUM(termin_nilai) AS jumlah WHERE YEAR(termin_tgl)=$pilih_tahun";
		$url_termin="http://jlt.lemigas.esdm.go.id/db2json.php?qu=SELECT%20SUM(termin_nilai_rp)%20AS%20jumlah%20FROM%20ws_termin%20WHERE%20YEAR(termin_tgl)=$pilih_tahun%20AND%20status=2";
		$get_url_termin = file_get_contents($url_termin);
		$data_json_termin = json_decode($get_url_termin);

		$url_realisasi="http://jlt.lemigas.esdm.go.id/db2json.php?qu=SELECT%20SUM(termin_nilai_rp)%20AS%20jumlah%20FROM%20ws_termin%20WHERE%20YEAR(termin_tgl)=$pilih_tahun%20AND%20status=3";
		...
		$data['target_lemigas'] = $data_json_target;
		...
		//$data['terkontrak'] = $this->db->query("SELECT SUM(nilai_kontrak) AS jumlah FROM kontrak WHERE YEAR(tgl_akhir) = $pilih_tahun")->row();
		$data['terkontrak'] = $this->db->query("SELECT SUM(t.jumlah) AS jumlah FROM termin AS t INNER JOIN kontrak AS k ON t.id_kontrak = k.id_kontrak WHERE YEAR(t.tgl_termin) <= $pilih_tahun")->row();
		$data['realisasi'] = $this->db->query("SELECT SUM(t.jumlah_realisasi) AS jumlah FROM termin AS t INNER JOIN kontrak AS k ON t.id_kontrak = k.id_kontrak AND YEAR(t.tgl_pembayaran) = $pilih_tahun AND t.status_pembayaran=1")->row();
		$data['result_realisasi'] = $this->db->query("SELECT SUM(t.jumlah_realisasi) AS jumlah FROM termin AS t INNER JOIN kontrak AS k ON t.id_kontrak = k.id_kontrak AND YEAR(t.tgl_pembayaran) < $pilih_tahun AND t.status_pembayaran=1")->row();
		$data['invoice'] = $this->db->query("SELECT SUM(t.jumlah) AS jumlah FROM termin AS t INNER JOIN kontrak AS k ON t.id_kontrak = k.id_kontrak AND YEAR(t.tgl_invoice) = $pilih_tahun AND t.status_cetak_invoice=1")->row();
		$data['pengeluaran'] = $this->db->query("SELECT SUM(p.jumlah_realisasi) AS jumlah FROM pengajuan AS p INNER JOIN rencana_operasional AS ro ON p.id_ro = ro.id_ro INNER JOIN kontrak AS k ON ro.id_kontrak = k.id_kontrak AND p.status_realisasi = 1 AND YEAR(p.tgl_pengajuan) = $pilih_tahun")->row();
		$data['pengeluaran_rkakl'] = $this->db->query("SELECT SUM(p.jumlah_realisasi) AS jumlah FROM pengajuan_rkakl AS p INNER JOIN detail_rkakl AS dr ON p.id_detail_rkakl = dr.id INNER JOIN rkakl AS r ON dr.id_rkakl = r.id_rkakl INNER JOIN rumah_layanan AS rl ON r.id_layanan = rl.id_rumah_layanan AND p.status_realisasi = 1 AND YEAR(p.tgl_pengajuan) = $pilih_tahun")->row();
		$data['pelaksana_layanan'] = $this->db->query("SELECT id_rumah_layanan,nama FROM rumah_layanan")->result();
		$data['satker'] = $this->db->query("SELECT * FROM satker WHERE id_satker > 0")->result();
		$data['target'] = $this->m_kontrak->sum_target_all($pilih_tahun)->row();
