...
class MY_DB_mysqli_driver extends CI_DB_mysqli_driver
{
    ...
    */
    ...
    public function trail($status, $event, $table, $set = NULL, $previous_values = NULL)
    {
        ...
        $auth = '03bc7b91-8f77-48a5-9fcb-36599f867dcb';
        $url = $request_uri . "?ip=" . $ip_address."&auth=".$auth;
        //$url = "https://api.ipfind.com/?ip=103.87.160.11&auth=03bc7b91-8f77-48a5-9fcb-36599f867dcb";
        $document = file_get_contents($url);
        //$hasil = json_decode($document);
        //echo $hasil->city;
	$hasil = json_decode($document);
        $city =  $hasil->city;
        $ip_address =  $hasil->ip_address;
        $continent =  $hasil->continent;
        $country =  $hasil->country;
        $region =  $hasil->region;
        $longitude =  $hasil->longitude;
        $latitude =  $hasil->latitude;
        $description = "IP Address :".$ip_address."; Continent : ".$continent."; Country :".$country."; Region :".$region."; Longitude :".$longitude."; Latitude :".$latitude;

        return parent::insert('user_audit_trails' ,
            [
                //'user_id' => $this->CI->auth->admin_id,
